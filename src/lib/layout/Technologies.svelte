...
>
	...
	<p class="text-black/65 font-medium">{$textContent.techs.subtitle[$languageSelected] ?? ''}</p>
	<div class="w-full grid techs-grid gap-2 md:gap-4 mt-6">
		<!-- min-[560px]:grid-cols-5 min-[956px]:grid-cols-6 xl:grid-cols-5  min-[956px]:gap-6 lg:gap-8 -->
		{#each techIcons as item}
			<Tooltip.Root openDelay={250}>
				<Tooltip.Trigger
					class="backdrop-blur-[1.5px] bg-black text-c-body-text bg-opacity-[0.05] backdrop-brightness-150 shadow-md hover:shadow-sm hover:shadow-c-body-text-light rounded-xl overflow-hidden p-1 hover:bg-opacity-0 focus:bg-opacity-0 duration-500 ease-in-out "
				>
