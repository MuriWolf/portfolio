...
<main class="max-w-[1340px] mx-auto flex flex-col gap-4 xs:gap-8 my-4 xs:my-8">
    <section class="mx-4 xs:mx-8 rounded-2xl py-12 px-4 xs:px-6 sm:px-8 lg:px-16  shadow-xl shadow-stone-900 gradient-blue-ish">
      <div class="relative z-10 max-w-[800px] bg-gradient-to-t from-[#f7f7f715] to-[#f7f7f75b] backdrop-blur-sm bg-clip-padding border-0 p-0 sm:px-3 rounded-2xl flex flex-col mx-auto shadow-md shadow-black/20 {project.images.length == 1 ? 'sm:py-3' : 'sm:pt-7 sm:pb-4'}">
        <div class="flex items-center gap-6 mb-3">
            <a href="/#projects" class="bg-black/50 hover:brightness-90 active:scale-95 transition-all w-[40px] h-[40px] justify-center items-center rounded-full hidden md:flex">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" fill="white" width="24" height="24"><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l192 192c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L77.3 256 246.6 86.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-192 192z"/></svg>
            </a>
            <h1 class="flex-1 text-white text-center font-title font-semibold text-shadow-sm text-lg max-xs:uppercase xs:text-xl sm:text-2xl md:text-3xl  bg-opacity-75 p-2 rounded-lg mr-12">{project.title[$languageSelected]}</h1>
        </div>
        {#if project.images.length == 1}  
          <div class="mx-auto rounded-2xl overflow-hidden inline-block shadow-custom-lg shadow-black/30 ">
              <img src={project.images[0]} alt={`${project.title[$languageSelected]} project screenshot`} height="636" width="736" class="block mx-auto w-full" >
          </div>
        {:else}
          <Carousel.Root 
            bind:api
          ...
          <footer class="p-2 mt-2 flex items-center justify-center gap-4 max-xs:px-4 pb-0">
              ...
              <div class="flex gap-4 items-center justify-center">
                {#each project.images as image (project.images.indexOf(image)) }
                    <button on:click={() => api.scrollTo(project.images.indexOf(image))} class="w-4 h-4 border-2 border-white transition-all duration-300 rounded-full {current == project.images.indexOf(image) +1 ? ' bg-white' : ''}"></button>
                {/each}
              </div>
    ...
    <section class="relative bg-black py-12 px-8 lg:px-16 rounded-2xl mx-4 xs:mx-8 text-center  shadow-xl shadow-stone-900">
      ...
      {/if}
        ...
        <div class="flex max-sm:flex-col justify-center gap-6 items-center max-w-lg mx-auto mb-12">
          <a href={project.liveUrl} class="transition-all duration-300 {project.codeUrl === "private" ? 'w-full sm:w-3/5 hover:w-full' : 'w-full flex-[2] hover:flex-[3] focus-within:flex-[3]'}" target="_blank">
            <Button class="bg-white w-full block text-black font-title font-bold text-lg xs:text-xl rounded-[5px]  active:bg-white hover:bg-white hover:brightness-90 active:scale-95 transition-all h-10 shadow-inner-sm shadow-black">Live View</Button>
          </a>
          {#if !(project.codeUrl === "private")}
            <a href={project.codeUrl} class="flex-[2] hover:flex-[3] focus-within:flex-[3] transition-all duration-300 w-full" target="_blank">
              ...
              <Button class="flex items-center h-10 pb-1 bg-transparent mx-auto hover:w-full text-white border-2 border-white font-title font-medium text-lg rounded-[5px]  shadow-[#00000066] active:bg-transparent hover:bg-transparent hover:brightness-90 active:scale-95 transition-all duration-300 w-full">Discover More</Button>
            </a>
          {/if}
        </div>
        ...
        <div class="flex justify-center gap-6 items-center max-w-lg mx-auto ">
          <a href="/#projects" class="flex-1 w-full -mt-5">
            <Button class="flex items-center h-10 pb-1 bg-transparent mx-auto hover:w-full text-white font-title font-medium text-lg rounded-[5px]  shadow-[#00000066] active:bg-transparent hover:bg-transparent hover:brightness-90 active:scale-95 transition-all duration-300 w-full sm:w-3/5">See Other Projects </Button>
          </a>
        </div>
    </section>
</main>
